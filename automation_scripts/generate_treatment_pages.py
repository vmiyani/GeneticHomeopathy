...
# Data structure for the 7 treatment categories
categories = [
    {
        "id": "skin",
        ...
        "intro_html": "<p>Dr. Riddhi Mayani treats skin conditions through a holistic approach. Conventional medicine often relies on topical steroids and ointments that merely suppress symptoms, pushing the inflammation deeper. Homeopathy believes that the skin is a mirror of internal health.</p><p>By understanding your entire medical history, stress levels, immunity, and genetics, we prescribe gentle, individualized remedies that aim to resolve the root disturbance. This approach offers safe, long-lasting relief from persistent skin disorders without harmful side effects.</p>",
    ...
    },
    {
        ...
        "causes": [
            ...
            "Disruption of the gut microbiome (dysbiosis) from antibiotics",
    ...
    },
    {
        ...
        "subtitle": "Gentle, comprehensive care for psychological well-being that tackles root emotional traumas without chemical dependency.",
        "intro_html": "<p>Mental health is a vital constituent of an individual's general well-being. Dr. Riddhi Mayani takes a deeply holistic approach to mental and emotional disorders. Contrary to conventional procedures that often forcefully sedate or temporarily mask symptoms, homeopathy aims at healing the mind-body connection from its root.</p><p>Homeopathy works by evaluating your state of health, emotions, specific fears, and precise mind-frame. By utilizing natural micro-doses, it gently activates the body's intrinsic ability to balance neurotransmitters and process unresolved mental blockages.</p>",
        "what_is_html": "<p>Mental diseases are conditions of the mind that fundamentally alter a person’s mood, emotional processing, cognitive thoughts, and behavior. These states can severely reduce an individual’s ability to handle everyday tasks, maintain relationships, and sustain stability.</p><p>Common conditions targeted by homeopathy include Generalized Anxiety, Clinical Depression, OCD, Panic Attacks, PTSD, and severe Insomnia. Homeopathic medicine has historically shown profound efficacy in treating these states gently and without the withdrawal symptoms associated with conventional psychiatric drugs.</p>",
        "symptoms": [
    ...
    },
    {
        ...
        "subtitle": "Manage chronic pain, stiffness, and degenerative conditions naturally, reducing the need for long-term NSAID medication.",
        "intro_html": "<p>Joint and musculoskeletal pain can strip away your mobility and joy. Conventional treatments frequently rely heavily on painkillers, NSAIDs, and steroids that carry a heavy burden of long-term side effects. At GeneticHomeopathy, Dr. Riddhi Mayani provides a profoundly safer alternative.</p><p>Homeopathic therapies focus on reducing systemic inflammation, slowing down joint degeneration, and improving the absolute mobility of the patient. The individualized remedies uniquely target precise types of pain—whether it is tearing, aching, burning, or stiffness worst in the morning.</p>",
    ...
    },
    {
        ...
        "intro_html": "<p>Children require exceptionally mild yet deeply effective medical care. Dr. Riddhi Mayani specializes in homeopathic pediatric treatments because they are inherently safe, non-toxic, and devoid of the harsh side effects associated with frequent antibiotic or steroid use.</p><p>Because homeopathic pills are sweet and easily dissolved, they are simple to administer even to infants. Our treatments aim not only to resolve the immediate acute infection or behavioral issue but to fundamentally strengthen the child's developing immune and nervous systems.</p>",
...
for cat in categories:
  ...
  <section class="section section--sage" id="cta-section">
    <div class="container">
      <div class="cta-banner fade-in" style="background: var(--primary); color: var(--white); border-radius: var(--radius-xl); padding: var(--space-12) var(--space-6); text-align: center; box-shadow: var(--shadow-lg);">
        <h2 style="color: var(--white); margin-bottom: var(--space-4);">Ready to Find Natural Relief?</h2>
        <p style="font-size: 1.1rem; max-width: 600px; margin: 0 auto var(--space-8); opacity: 0.9;">Book a consultation to receive a highly individualized homeopathic treatment plan targeting the root cause of your {cat["title_lower"]} conditions.</p>
        <div style="display: flex; gap: var(--space-4); justify-content: center; flex-wrap: wrap;">
          <a href="contact.html" class="btn btn--white btn--lg" style="background: var(--white); color: var(--primary); font-weight: 600;">Book Your Consultation</a>
          <a href="tel:+18258890473" class="btn btn--outline btn--lg" style="background: transparent; color: var(--white); border-color: var(--white);">Call Us Now</a>
        </div>
      </div>
    </div>
  </section>
...
# 3. Update we-treat.html to add "Learn More" buttons
print("Updating we-treat.html with Learn More buttons...")
for cat in categories:
    target_str = f'<!-- {cat["title"].split()[0]}'
    # We will search for the <div class="treat-expand__list"> block and append a button after it.
    import re
    # Match the specific section by data-category
    pattern = r'(<div class="treat-category-section[^>]*data-category="' + cat["id"] + r'"[^>]*>.*?</div>\s*</div>)'
    ...
    if match:
        ...
        # Inject the button right before the final closing </div> of that category section
        new_block = original_block[:original_block.rfind('</div>')] + button_html + '\n      </div>'
        base_html = base_html.replace(original_block, new_block)

with open('we-treat.html', 'w', encoding='utf-8') as f:
    f.write(base_html)
print("we-treat.html updated.")
